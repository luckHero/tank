...
public class BulletTankCollider implements Collider {
    ...
    public boolean collider(GameObject o1, GameObject o2) {
        if (o1 instanceof Bullet && o2 instanceof Tank) {
            ...
            Tank tank = (Tank) o2;
            if (collideWith(tank, bullet)) {//子弹与坦克相撞
                return false;//返回fas
            }
    ...
    //子弹和坦克进行碰撞检测
    public boolean collideWith(Tank tank, Bullet bullet) {
        if (bullet.getGroup() == tank.getGroup()) return false; //判断子弹的group 和坦克的group
...
//        Rectangle tankRect = new Rectangle(tank.getX(), tank.getY(), Tank.WIDTH, Tank.HEIGHT); //坦克的矩形
        if (bullet.rectangle.intersects(tank.getRectangle())) { //判断子弹是否与坦克相交
            tank.die();//坦克死亡
            bullet.die();//子弹死亡
